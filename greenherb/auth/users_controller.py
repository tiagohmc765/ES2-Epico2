...
def _get_current_user(credentials: Optional[HTTPAuthorizationCredentials]) -> dict:
    ...
    return jwt_provider.decode(credentials.credentials.strip())


@router.get("")
def list_users(credentials: Optional[HTTPAuthorizationCredentials] = Depends(_bearer)):
    try:
...
def create_user(payload: dict, credentials: Optional[HTTPAuthorizationCredentials] = Depends(_bearer)):
    try:
        current_user = _get_current_user(credentials)
        require_role(current_user, ["ADMIN"])
...
@router.get("/{user_id}")
def get_user(user_id: int, credentials: Optional[HTTPAuthorizationCredentials] = Depends(_bearer)):
    try:
        current_user = _get_current_user(credentials)
        require_role(current_user, ["ADMIN"])
...
def delete_user(user_id: int, credentials: Optional[HTTPAuthorizationCredentials] = Depends(_bearer)):
    try:
        current_user = _get_current_user(credentials)
        require_role(current_user, ["ADMIN"])
